...
bool CRetroPlayerAudio::OpenStream(PCMFormat format, double sampleRate, const AudioChannelMap& channelMap)
{
  const AEDataFormat pcmFormat = CAudioTranslator::TranslatePCMFormat(format);
  if (pcmFormat == AE_FMT_INVALID)
  {
    CLog::Log(LOGERROR, "RetroPlayer[AUDIO]: Unknown PCM format: %d", static_cast<int>(format));
    return false;
  }

  unsigned int iSampleRate = static_cast<unsigned int>(std::round(sampleRate));
  if (iSampleRate == 0)
  {
    CLog::Log(LOGERROR, "RetroPlayer[AUDIO]: Invalid samplerate: %f", sampleRate);
    return false;
  }
