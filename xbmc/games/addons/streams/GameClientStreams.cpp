...
IGameClientStream *CGameClientStreams::OpenStream(const game_stream_properties &properties)
{
  ...
  RETRO::StreamType streamType;
  if (CGameClientTranslator::TranslateStreamType(properties.type, streamType))
  {
    RETRO::StreamPtr retroStream = m_streamManager->OpenStream(streamType);
    if (retroStream)
    {
      ...
      {
        gameStream.reset(new CGameClientStreamAudio(std::move(retroStream), properties.audio, m_gameClient.GetSampleRate()));
        break;
      }
      case GAME_STREAM_VIDEO:
      {
        gameStream.reset(new CGameClientStreamVideo(std::move(retroStream), properties.video));
        break;
      }
      default:
        break;
...
void CGameClientStreams::CloseStream(IGameClientStream *stream)
{
  if (stream != nullptr)
  {
    m_streamManager->CloseStream(stream->ReleaseStream());
    delete stream;
  }
}
